    ...
    MessagingContext
{
    ...
    function verifiable(Origin calldata _origin, address _receiver, address _receiveLib, bytes32 _payloadHash)
        external
        view
        returns (bool)
    {
    ...
    function executable(Origin calldata _origin, address _receiver) external view returns (ExecutionState) {
        ...
        if (payloadHash != NIL_PAYLOAD_HASH && _origin.nonce <= inboundNonce(_receiver, _origin.srcEid, _origin.sender))
        {
