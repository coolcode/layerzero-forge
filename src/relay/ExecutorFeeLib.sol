...
contract ExecutorFeeLib is Ownable, IExecutorFeeLib {
    ...
    function getFeeOnSend(FeeParams calldata _params, IExecutor.DstConfig calldata _dstConfig, bytes calldata _options)
        ...
        returns (uint256 fee)
    {
        (uint256 totalDstAmount, uint256 totalGas) =
            _decodeExecutorOptions(_isV1Eid(_params.dstEid), _dstConfig.baseGas, _dstConfig.nativeCap, _options);
    ...
    function _decodeExecutorOptions(bool _v1Eid, uint64 _baseGas, uint128 _nativeCap, bytes calldata _options)
        internal
        pure
