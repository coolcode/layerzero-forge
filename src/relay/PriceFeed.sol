...
import { ILayerZeroEndpointV2 } from "src/protocol/interfaces/ILayerZeroEndpointV2.sol";
...
contract PriceFeed is ILayerZeroPriceFeed, OwnableUpgradeable, Proxied {
    ...
    function estimateFeeOnSend(uint32 _dstEid, uint256 _callDataSize, uint256 _gas)
        external
        payable
        returns (uint256, uint128, uint128, uint128)
    {
    ...
    // NOTE: to be reverted when endpoint is in sendContext
    function estimateFeeByEid(uint32 _dstEid, uint256 _callDataSize, uint256 _gas)
        external
        view
        returns (uint256, uint128, uint128, uint128)
    {
    ...
    // NOTE: to be reverted when endpoint is in sendContext
    // NOTE: to support legacy
    function estimateFeeByChain(uint16 _dstEid, uint256 _callDataSize, uint256 _gas)
        external
        view
        returns (uint256 fee, uint128 priceRatio)
    {
    ...
    function _estimateFeeWithDefaultModel(uint32 _dstEid, uint256 _callDataSize, uint256 _gas)
        internal
        view
        returns (uint256 fee, uint128 priceRatio)
    {
    ...
    function _estimateFeeWithOptimismModel(uint32 _dstEid, uint256 _callDataSize, uint256 _gas)
        internal
        view
        returns (uint256 fee, uint128 priceRatio)
    {
    ...
    function _estimateFeeWithArbitrumModel(uint32 _dstEid, uint256 _callDataSize, uint256 _gas)
        internal
        view
        returns (uint256 fee, uint128 priceRatio)
    {
        ...
        uint256 gasForL1CallData =
            ((_callDataSize * ARBITRUM_COMPRESSION_PERCENT) / 100) * _arbitrumPriceExt.gasPerL1CallDataByte;
        // L2 Fee
        uint256 gasForL2CallData = _callDataSize * arbitrumPrice.gasPerByte;
        uint256 gasFee =
            (_gas + _arbitrumPriceExt.gasPerL2Tx + gasForL1CallData + gasForL2CallData) * arbitrumPrice.gasPriceInUnit;
