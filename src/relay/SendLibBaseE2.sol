...
import { ILayerZeroEndpointV2, MessagingFee } from "src/protocol/interfaces/ILayerZeroEndpointV2.sol";
import { IMessageLib, MessageLibType } from "src/protocol/interfaces/IMessageLib.sol";
import { ISendLib, Packet } from "src/protocol/interfaces/ISendLib.sol";
...
abstract contract SendLibBaseE2 is SendLibBase, ERC165, ISendLib {
    ...
    constructor(address _endpoint, uint256 _treasuryGasLimit, uint256 _treasuryNativeFeeCap)
        SendLibBase(_endpoint, ILayerZeroEndpointV2(_endpoint).eid(), _treasuryGasLimit, _treasuryNativeFeeCap)
    { }

    function supportsInterface(bytes4 _interfaceId) public view virtual override(ERC165, IERC165) returns (bool) {
        return _interfaceId == type(IMessageLib).interfaceId || _interfaceId == type(ISendLib).interfaceId
        ...
        returns (MessagingFee memory, bytes memory)
    {
        ...
        (uint256 treasuryNativeFee, uint256 lzTokenFee) =
            _payTreasury(_packet.sender, _packet.dstEid, totalNativeFee, _payInLzToken);
        ...
        returns (MessagingFee memory)
    {
        (uint256 nativeFee, uint256 lzTokenFee) =
            _quote(_packet.sender, _packet.dstEid, _packet.message.length, _payInLzToken, _options);
    ...
    function _payWorkers(Packet calldata _packet, bytes calldata _options)
        internal
        returns (bytes memory encodedPacket, uint256 totalNativeFee)
    {
